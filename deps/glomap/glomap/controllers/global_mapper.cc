...
bool GlobalMapper::Solve(const colmap::Database& database,
  ...
  if (!ofs) {
    ...
    //   std::cout << image.features.size() << std :: endl;
    // }
    uint32_t idx = 0;
    uint32_t sum = 0;
    for (const auto& [track_id, track] : tracks) {
      idx++;
      sum += track.observations.size();
      for(const auto& observation :track.observations){
        ofs<<observation.first<<" "<<images.at(observation.first).features[observation.second][0]<<" "<<images.at(observation.first).features[observation.second][1]<<" "<<idx<<std::endl;
      }
    }
  ...
  if (!ofn) {
    ...
    for (const auto& [image_id, image] : images) {
      ofn<<image_id<<" "<<image.file_name<<std::endl;
      if(!image.is_registered){
        std::cout << "image_id: " << image_id << " is not registered" << std::endl;
      }
    }
    ofn.close();
  }
  ...
  if (!ofc) {
    ...
    for (const auto& [pair_id, image_pair] : view_graph.image_pairs) {
      if(image_pair.is_valid){
        auto quat = image_pair.cam2_from_cam1.rotation;
        auto trans = image_pair.cam2_from_cam1.translation;
        
        ofc<<image_pair.image_id1<<" "<<image_pair.image_id2<<" "<<quat.w()<<" "<<quat.x()<<" "<<quat.y()<<" "<<quat.z()<<" "<<trans(0)<<" "<<trans(1)<<" "<<trans(2)<<std::endl;
      }
    }
    ofc.close();
  }
